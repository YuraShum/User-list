...
export default function User({user, onClickInvite, isInvited}){
    const styles = {width: 28, height: 28, opacity: 0.3,
        cursor: "pointer",
        marginRight: 5};
    ...
    <li >
        ...
        {isInvited ? <AiOutlineMinus onClick = {() => onClickInvite(user.id)} style={styles}/> : <AiOutlinePlus onClick = {() => onClickInvite(user.id)} style={styles}/>}
